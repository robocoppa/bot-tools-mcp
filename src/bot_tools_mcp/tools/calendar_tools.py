...
from bot_tools_mcp._util import parse_iso
...
def register(mcp: FastMCP, identity: Identity, smtp: SmtpConfig | None = None):
    ...
    def _creds(ctx: Context) -> tuple[str, str]:
        bot = current_bot(ctx)
        return bot, identity.radicale_password(bot)
    ...
    async def list_events(ctx: Context, start: str = "", end: str = "") -> list[dict]:
        ...
        try:
            events = await asyncio.to_thread(
                cal.list_events, url, bot, pw, start=s, end=e
            )
        except cal.CalDavError as exc:
            raise ToolError(str(exc)) from exc
    ...
    async def delete_event(ctx: Context, event_id: str) -> str:
        ...
        try:
            await asyncio.to_thread(
                cal.delete_event, url, bot, pw, event_id=event_id.strip()
            )
        except cal.CalDavError as exc:
            raise ToolError(str(exc)) from exc
    ...
    async def send_calendar_invite(
        ...
        try:
            await asyncio.to_thread(cal.save_ics, url, bot, pw, ics=ics)
        except cal.CalDavError as exc:
            raise ToolError(str(exc)) from exc
        # 2. email the invite (fail loud, name the smarthost)
        ...
        try:
            ...
            raise ToolError(
                f"invite saved but email failed via {smtp_config.host}:{smtp_config.port}: {exc}"
            ) from exc
